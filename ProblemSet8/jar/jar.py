...
# cookies. In a file called jar.py, implement a class called Jar with 
# these methods:
    ...
    # __str__ should return a str with  🍪, where  is the number of 
    # cookies in the cookie jar. For instance, if there are 3 cookies in 
...
class Jar:
    ...
    def withdraw(self, n):
        if self._capacity < n:
            raise ValueError('The number of cookies to removes is greater than its capacity')
        if self._size < n:
            raise ValueError('There are not enought cookies to remove')
        self._size -= n
